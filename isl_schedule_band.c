	...
	__isl_take isl_schedule_band *band, __isl_take isl_pw_multi_aff *pma)
{
	...
	ctx = isl_schedule_band_get_ctx(band);
	space1 = isl_multi_union_pw_aff_get_space(band->mupa);
	space2 = isl_pw_multi_aff_get_domain_space(pma);
	space1 = isl_space_align_params(space1, isl_space_copy(space2));
	space2 = isl_space_align_params(space2, isl_space_copy(space1));
	...
	if (!equal)
		isl_die(ctx, isl_error_invalid, "spaces don't match",
			goto error);
	band->mupa = isl_multi_union_pw_aff_apply_pw_multi_aff(band->mupa, pma);
	if (!band->mupa)
		return isl_schedule_band_free(band);

	band->permutable = 0;
	free(band->coincident);
	band->n = isl_multi_union_pw_aff_dim(band->mupa, isl_dim_set);
	band->coincident = isl_calloc_array(ctx, int, band->n);
